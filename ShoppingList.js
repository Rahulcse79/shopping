    ...
    const SuggestionAPICall = async () => {
        ...
        try {
            ...
            products.forEach(product => {
                const matchedItem = data.data.find(item => item.id === product.id && product.quantity > 0);
                if (matchedItem) {
                    makeFormatedData.push(product);
                }
            });
            setSuggestion(makeFormatedData);
        } catch (error) {
                        ...
                        <tbody>
                            {products.map((product, index) => (
                                <tr key={product.id}>
                                    ...
                                    <td>
                                        <button onClick={() => handleRemoveProduct(product)} className="remove-button">Remove</button>
                                        <button onClick={() => handleAddProduct(product)} className="add-button">Add</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
